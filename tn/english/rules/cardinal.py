...
class Cardinal(Processor):
    def __init__(self, deterministic: bool = False):
        ...
        """
        super().__init__('cardinal', ordertype="tn")
    ...
    def build_tagger(self):
        ...
        self.graph_hundred_component_at_least_one_none_zero_digit = (
            pynini.closure(self.DIGIT, 2, 3) | pynini.difference(self.DIGIT, pynini.accep("0"))
        ) @ graph
        ...
        self.single_digits_graph = single_digits_graph + pynini.closure(self.INSERT_SPACE + single_digits_graph)
        ...
        if not self.deterministic:
            ...
            self.single_digits_graph |= single_digits_graph_oh + pynini.closure(self.INSERT_SPACE + single_digits_graph_oh)

            single_digits_graph_with_commas = pynini.closure(
                self.single_digits_graph + self.INSERT_SPACE, 1, 3
            ) + pynini.closure(
                pynutil.delete(",")
                + single_digits_graph
                + self.INSERT_SPACE
        ...
        graph = (
            ...
            + (pynini.closure(pynutil.delete(",") + self.DIGIT ** 3) | pynini.closure(self.DIGIT ** 3))
        ) @ graph
        ...
        if self.deterministic:
            ...
            self.long_numbers = plurals._priority_union(long_numbers, self.graph_with_and, pynini.closure(self.VCHAR)).optimize()
            cardinal_with_leading_zeros = pynini.compose(
                pynini.accep("0") + pynini.closure(self.DIGIT), self.single_digits_graph
            )
            ...
            cardinal_with_leading_zeros = (
                leading_zeros + self.INSERT_SPACE + pynini.compose(pynini.closure(self.DIGIT), self.graph_with_and)
            )
            ...
            final_graph |= pynini.compose(final_graph, one_to_a_replacement_graph.optimize() + pynini.closure(self.VCHAR)).optimize()
    ...
    def add_optional_and(self, graph):
        ...
        no_hundred = pynini.difference(pynini.closure(self.VCHAR), not_quote + pynini.accep("hundred") + not_quote).optimize()
        integer |= (
        ...
        optional_hundred = pynini.compose(optional_hundred, pynini.closure(self.VCHAR) + pynini.cross(" hundred", "") + pynini.closure(self.VCHAR))
