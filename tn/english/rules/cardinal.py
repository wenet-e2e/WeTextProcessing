...
class Cardinal(Processor):
    ...
    def build_tagger(self):
        ...
        graph = pynini.Far(get_abs_path("english/data/number/cardinal_number_name.far")).get_fst()
        graph_au = pynini.Far(get_abs_path("english/data/number/cardinal_number_name_au.far")).get_fst()
        self.graph_hundred_component_at_least_one_none_zero_digit = (pynini.closure(self.DIGIT, 2, 3) | pynini.difference(
            self.DIGIT, pynini.accep("0"))) @ graph
        ...
        self.single_digits_graph = single_digits_graph + (self.INSERT_SPACE + single_digits_graph).star
        ...
        if not self.deterministic:
            ...
            single_digits_graph_with_commas = (
                ...
                (pynutil.delete(",") + single_digits_graph + self.INSERT_SPACE + single_digits_graph + self.INSERT_SPACE +
                 single_digits_graph + (self.INSERT_SPACE + pynutil.delete(",") + single_digits_graph + self.INSERT_SPACE +
                                        single_digits_graph + self.INSERT_SPACE + single_digits_graph).star))

        graph = (pynini.closure(self.DIGIT, 1, 3) + ((pynutil.delete(",") + self.DIGIT**3).star |
        ...
        if self.deterministic:
            long_numbers = pynini.compose(self.DIGIT**(5, ...), self.single_digits_graph).optimize()
            self.long_numbers = plurals._priority_union(long_numbers, self.graph_with_and, self.VCHAR.star).optimize()
            cardinal_with_leading_zeros = pynini.compose(pynini.accep("0") + self.DIGIT.star, self.single_digits_graph)
            final_graph = self.long_numbers | cardinal_with_leading_zeros
            final_graph |= self.add_optional_and(graph_au)
        else:
            leading_zeros = pynini.compose(pynini.accep("0").plus, self.single_digits_graph)
            cardinal_with_leading_zeros = (leading_zeros + self.INSERT_SPACE +
            ...
            one_to_a_replacement_graph = (pynutil.add_weight(pynini.cross("one hundred", "a hundred"), 0.0001)
                                          | pynutil.add_weight(pynini.cross("one thousand", "thousand"), -0.0001)
                                          | pynutil.add_weight(pynini.cross("one million", "a million"), 0.0001))
            final_graph |= pynini.compose(
                final_graph,
                one_to_a_replacement_graph.optimize() + self.VCHAR.star,
            ).optimize()
            # remove commas for 4 digits numbers
            four_digit_comma_graph = (self.DIGIT - "0") + pynutil.delete(",") + self.DIGIT**3
            final_graph |= pynini.compose(four_digit_comma_graph.optimize(), final_graph).optimize()

        self.final_graph = final_graph.optimize()
        optional_minus_graph = (self.tag_field("negative", pynini.cross("-", "true")) + self.INSERT_SPACE).ques
        final_graph = optional_minus_graph + self.tag_field("integer", self.final_graph)
    ...
    def add_optional_and(self, graph):
        ...
        no_thousand_million = pynini.difference(not_quote,
                                                not_quote + pynini.union("thousand", "million") + not_quote).optimize()
        integer = (not_quote +
                   pynutil.add_weight(pynini.cross("hundred ", "hundred and ") + no_thousand_million, -0.0001)).optimize()

        no_hundred = pynini.difference(self.VCHAR.star, not_quote + pynini.accep("hundred") + not_quote).optimize()
        integer |= (not_quote +
                    pynutil.add_weight(pynini.cross("thousand ", "thousand and ") + no_hundred, -0.0001)).optimize()
    ...
    def build_verbalizer(self):
        ...
        Finite state transducer for verbalizing cardinal, e.g.
            cardinal { negative: "-" integer: "23" } -> minus twenty three
        """
        optional_sign = pynini.cross('negative: "-"', "minus ")
        if not self.deterministic:
            optional_sign |= pynutil.add_weight(pynini.cross('negative: "-"', "negative "), -0.0001)
            optional_sign |= pynini.cross('negative: "-"', "dash ")

        self.optional_sign = (optional_sign + self.DELETE_SPACE).ques

        self.integer = self.DELETE_SPACE + pynutil.delete('"') + self.final_graph + pynutil.delete('"')
