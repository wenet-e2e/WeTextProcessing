...
class Word(Processor):
    ...
    def __init__(self, deterministic: bool = False):
        ...
        """
        super().__init__("word", ordertype="tn")
        ...
        emphasis = (
            pynini.accep("<")
            + (
                (pynini.closure(self.NOT_SPACE - pynini.union("<", ">"), 1) + pynini.closure(pynini.accep("/"), 0, 1))
                | (pynini.accep("/") + pynini.closure(self.NOT_SPACE - pynini.union("<", ">"), 1))
            )
            + pynini.accep(">")
        )
        punct = plurals._priority_union(emphasis, punct, pynini.closure(self.VCHAR))
    ...
    def build_tagger(self):
        ...
        """
        default_graph = pynini.closure(pynini.difference(self.NOT_SPACE, self.punct_graph.project("input")), 1)
        symbols_to_exclude = (pynini.union("$", "€", "₩", "£", "¥", "#", "%") | self.DIGIT).optimize()
        graph = pynini.closure(pynini.difference(self.NOT_SPACE, symbols_to_exclude), 1)
        graph = pynutil.add_weight(graph, self.MIN_NEG_WEIGHT) | default_graph

        # leave phones of format [HH AH0 L OW1] untouched
        phoneme_unit = pynini.closure(self.ALPHA, 1) + pynini.closure(self.DIGIT)
        phoneme = (
        ...
        ipa_phoneme_unit = pynini.string_file(get_abs_path("english/data/whitelist/ipa_symbols.tsv"))
        # word in ipa form
        ipa_phonemes = (
        ...
        self.graph = plurals._priority_union(phoneme.optimize(), graph, pynini.closure(self.VCHAR))
        final_graph = (pynutil.insert("value: \"") + self.graph + pynutil.insert("\"")).optimize()
    ...
    def build_verbalizer(self):
        ...
        char = pynutil.delete("value:") + self.DELETE_SPACE + pynutil.delete("\"") + chars + pynutil.delete("\"")
        graph = char @ pynini.cdrewrite(pynini.cross(u"\u00A0", " "), "", "", pynini.closure(self.VCHAR))
