...
def get_quantity(decimal: 'pynini.FstLike',
    ...
    res = (pynutil.insert("integer_part: \"") + cardinal_up_to_hundred +
           pynutil.insert("\"") + pynutil.delete(" ").ques +
    ...
    if include_abbr:
        ...
        quantity = quantities
    res |= (decimal + pynutil.delete(" ").ques +
...
class Decimal(Processor):
    ...
    def build_tagger(self):
        ...
        optional_graph_negative = (pynutil.insert("negative: ") +
        ...
        if not self.deterministic:
            no_oh_zero = pynini.difference(
                self.VCHAR.star,
                (self.VCHAR.star + "oh" + self.VCHAR.star
                 + "zero" + self.VCHAR.star)
                | (self.VCHAR.star + "zero" + self.VCHAR.star + "oh"
                   + self.VCHAR.star),
            ).optimize()
            no_zero_oh = pynini.difference(
                self.VCHAR.star,
                self.VCHAR.star + pynini.accep("zero") +
                self.VCHAR.star + pynini.accep("oh") +
            ...
            self.final_graph_wo_negative |= pynini.compose(
                ...
                pynini.cdrewrite(
                    pynini.cross("integer_part: \"zero\"",
                                 "integer_part: \"oh\""),
                    self.VCHAR.star, self.VCHAR.star,
                    self.VCHAR.star),
            )
            self.final_graph_wo_negative = pynini.compose(
