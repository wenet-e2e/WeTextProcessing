...
class Cardinal(Processor):
    ...
    def build_tagger(self):
        ...
        hundred_digit = string_file(get_abs_path("japanese/data/number/hundred_digit.tsv"))
        sign = string_file(get_abs_path("japanese/data/number/sign.tsv"))
        dot = string_file(get_abs_path("japanese/data/number/dot.tsv"))
        ties = string_file(get_abs_path("japanese/data/number/ties.tsv"))
        graph_teen = string_file(get_abs_path("japanese/data/number/teen.tsv"))
        ...
        if self.enable_million:
            ten_thousand = ((thousand | hundred | teen | tens | digits) + delete("万") + (thousand
                                                                                         | addzero + hundred
                                                                                         | addzero**2 + tens
                                                                                         | addzero**2 + teen
                                                                                         | addzero**3 + digits
            ...
            # 二万 十万 三十四万
            ten_thousand = ((teen | tens | digits) + delete("万") + (thousand
                                                                    | addzero + hundred
                                                                    | addzero**2 + tens
                                                                    | addzero**2 + teen
                                                                    | addzero**3 + digits
        ...
        big_integer = sign.ques + (((ten_thousand_minus + accep("兆")).ques + ten_thousand_minus + accep("億") +
                                    (ten_thousand_minus + (accep("万") + ten_thousand_minus.ques).ques).ques)
                                   | (ten_thousand_minus + accep("兆") + (ten_thousand_minus + accep("億")).ques +
                                      (ten_thousand_minus + (accep("万") + ten_thousand_minus.ques).ques).ques))
        ...
        self.cardinal = self.apply_input_processor(cardinal, self.input_processor)
        self.tagger = self.add_tokens(self.tag_field("value", self.cardinal)).optimize()
